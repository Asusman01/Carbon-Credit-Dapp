...
describe("Carbon Credit", function () {
  ...
  it("should sell to another account", async function () {
    ...
    // acc2 buying it
    await expect(() =>
      carbonCredit.connect(acc2).buyCredit(creditId, { value: buyPrice })
    ...
    // royalties: acc1 gets 10%
    const creatorShare = Math.floor((new_price * 10) / 100);
    const ownerShare = new_price - creatorShare;

    await expect(() =>
      carbonCredit.connect(acc3).buyCredit(creditId, { value: newBuyPrice })
    ).to.changeEtherBalances([acc3, acc2, acc1], [
      -new_price,
      ownerShare,
      creatorShare,
    ]);
