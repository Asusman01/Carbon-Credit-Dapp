...
export const CCProvider = ({ children }) => {
   ...
   const buyCredit = async (creditId, price) => {
    try {
        ...
        // 1. Connect to ONINO token contract
        const oninoToken = new ethers.Contract(
            ONINO_TOKEN_ADDRESS,
            [
        ...
        // 2. Approve the CarbonCredit contract to spend ONINO
        const approvalTx = await oninoToken.approve(CC_ADDRESS, priceInTokens);
        await approvalTx.wait();

        // 3. Call buyCredit without sending value (ERC20 handles it)
        const transaction = await contract.buyCredit(creditId);
        const receipt = await transaction.wait();

        console.log("✅ Credit purchased with ONINO!", receipt);
        return receipt;
    } catch (error) {
